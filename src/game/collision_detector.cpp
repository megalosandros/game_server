...
CollectionResult TryCollectPoint(geom::Point2D a, geom::Point2D b, geom::Point2D c) {
    //
    // Проверим, что перемещение ненулевое - в функцию может попасть только 
    // движущийся сборщик.
    // Тут приходится использовать строгое равенство, а не приближённое,
    // пскольку при сборе трофеев придётся учитывать перемещение даже на небольшое
    // расстояние.
    //
    assert(b.x != a.x || b.y != a.y);

    //
    //  Расстояние от сборщика до предмета по осям
    //
    const double u_x = c.x - a.x;
    const double u_y = c.y - a.y;

    //
    //  На какое расстояние передвинулся сборщик
    //
...
std::vector<GatheringEvent> FindGatherEvents(
    ...
    static auto eq_pt = [](geom::Point2D p1, geom::Point2D p2) {
        //
        //   учитывайте перемещение на любое ненулевое расстояние — погрешностью можно пренебречь
        //
    ...
    for (size_t g = 0; g < provider.GatherersCount(); ++g) {
        ...
        for (size_t i = 0; i < provider.ItemsCount(); ++i) {
            ...
            if (collect_result.IsCollected(gatherer.width + item.width)) {
                GatheringEvent evt{.item_id = item.id, // i,
                                   .gatherer_id = gatherer.id, // g,
